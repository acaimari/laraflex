...
<link rel="stylesheet" href="/vendor/caimari/laraflex/assets/style.css">
...
<li class="control-section accordion-section open add-page" id="add-page">
    <h3 class="accordion-section-title hndle" tabindex="0"> System Page <span class="screen-reader-text">Press return or enter to expand</span></h3>
    <div class="accordion-section-content">
        <div class="inside">
            <div class="customlinkdiv" id="customlinkdiv">
                <p id="menu-item-url-wrap">
                    <label class="howto" for="system-page-menu-item-url">
                        <span>URL</span>&nbsp;&nbsp;&nbsp;
                        <select id="system-page-menu-item-url" name="url" class="menu-item-textbox">
                ...
                <p id="menu-item-name-wrap">
                    <label class="howto" for="system-page-menu-item-name">
                        <span>Label</span>&nbsp;
                        <input id="system-page-menu-item-name" name="label" type="text" class="regular-text menu-item-textbox input-with-default-title" title="Label menu">
                    </label>
                </p>

                <p class="button-controls">
                    <a href="#" onclick="addsystempagemenu()" class="button-secondary submit-add-to-menu right">Add menu item</a>
                    <span class="spinner" id="spincustomu"></span>
                </p>
            </div>
        </div>
...
function addsystempagemenu() {
    // Obtener los datos del menú desde el formulario
    var menuname = document.getElementById('menu-to-edit').getAttribute('id');
    var systemPageURL = document.getElementById('system-page-menu-item-url').value;
    console.log(systemPageURL); // Agregar esta línea
    var systemPageLabel = document.getElementById('system-page-menu-item-name').value;
    var systemPageID = document.getElementById('system-page-menu-item-url').options[document.getElementById('system-page-menu-item-url').selectedIndex].getAttribute('data-page-id');

    // Crear un nuevo elemento de menú
    var newItem = document.createElement('li');
    newItem.className = 'menu-item menu-item-depth-0 menu-item-page menu-item-edit-inactive pending'; 
    newItem.id = 'menu-item-new';

    // Crear los elementos hijos
    var dlElement = document.createElement('dl');
    dlElement.className = 'menu-item-bar';
    newItem.appendChild(dlElement);

    var dtElement = document.createElement('dt');
    dtElement.className = 'menu-item-handle';
    dlElement.appendChild(dtElement);

    var spanTitleElement = document.createElement('span');
    spanTitleElement.className = 'item-title';
    dtElement.appendChild(spanTitleElement);

    var spanLabelElement = document.createElement('span');
    spanLabelElement.className = 'menu-item-title';
    spanLabelElement.textContent = systemPageLabel;
    spanTitleElement.appendChild(spanLabelElement);

    // Agregar más elementos hijos aquí siguiendo tu estructura...

    // Agregar el nuevo elemento de menú al menú
    ...
    var data = {
        menuname: menuname,
        url: systemPageURL,
        label: systemPageLabel,
        page_id: systemPageID,
        // Agrega aquí cualquier otro dato que necesites enviar
    };

    console.log(data); // Agregar la impresión en consola de los datos a enviar

    // Enviar los datos al servidor con una solicitud POST
    fetch(updateitemr, { // Usar la variable que guarda la ruta correcta
        method: 'POST',
        headers: {
...
<script type="text/javascript" src="/vendor/caimari/laraflex/assets/scripts.js"></script>
<script type="text/javascript" src="/vendor/caimari/laraflex/assets/scripts2.js"></script>
<script type="text/javascript" src="/vendor/caimari/laraflex/assets/menu.js"></script>
@endauth
@endsection
